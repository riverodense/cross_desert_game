...
CONFIG_FILE = ROOT / "game_config.json"
SOLUTION_FILE = ROOT / "LAST_SOLUTION.json"
...
def load_config():
    ...
    # Generate master token if missing
    if not CONFIG.get("controller_master_token"):
    ...
    if LAST_SOLUTION:
        try:
            with open(SOLUTION_FILE, "w", encoding="utf-8") as f:
                json.dump(LAST_SOLUTION, f, indent=2, ensure_ascii=False)
        except Exception as e:
...
def require_controller_auth(master_required=False):
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not token:
                return jsonify({"error": "Missing X-Controller-Token header"}), 403
            
            master_token = CONFIG.get("controller_master_token", "")
            is_master = (token == master_token)
            
            if master_required and not is_master:
                return jsonify({"error": "Master token required"}), 403
            
            if CONFIG.get("controller_lock", False):
                if not is_master:
                    return jsonify({"error": "Controller locked. Only master token allowed."}), 403
            else:
                valid_tokens = CONFIG.get("controller_tokens", [])
                if not (is_master or token in valid_tokens):
                    return jsonify({"error": "Invalid controller token"}), 403
...
@app.get("/api/config")
def get_config():
    # Return config without sensitive master token  
    safe_config = {k: v for k, v in CONFIG.items() if k != "controller_master_token"}
...
def get_solution():
    if not CONFIG.get("show_solution_to_players", False):
        return jsonify({"error": "Solution not revealed to players"}), 403
...
def access_init():
    # Return master token for initial setup
    return jsonify({"master_token": CONFIG["controller_master_token"]})
...
def access_check():
    ...
    master_token = CONFIG.get("controller_master_token", "")
    is_master = (token == master_token)
    is_valid = is_master or token in CONFIG.get("controller_tokens", [])
